...
grd_check_block_errors_omp(wfc_blocks_ptr blocks) {
    ...
    #pragma omp parallel for num_threads(blocks->grid_side)
    for (uint32_t gy = 0; gy < blocks->grid_side; gy++) {
        for (uint32_t gx = 0; gx < blocks->grid_side; gx++) {
            ...
            for (uint32_t i = 0; i < block_size; i++) {
                if (blk[i] == 0 || (blk[i] & collapsed_mask) != 0) {
                    fprintf(stderr,"Failed check in block(%d,%d,%d)\n",gx,gy,i);
                    err_blk = false;
...
check_grid_omp(const wfc_blocks_ptr blocks)
{
    ...
    //     {
    //     #pragma omp section 
    //     err_blk = grd_check_block_errors(blocks);
    //     #pragma omp section 
    //     err_row = grd_check_row_errors(blocks);
    //     #pragma omp section
    //     err_col = grd_check_column_errors(blocks);
    //     }
    // }
    ...
    omp_set_num_threads(blocks->grid_side);
    #pragma omp parallel
    {
    ...
    #pragma omp for nowait collapse(2) schedule(runtime)
    for (uint32_t gy = 0; gy < blocks->grid_side; gy++) {
        for (uint32_t y = 0; y < blocks->block_side; y++) {
            uint64_t collapsed_mask = 0;
            // printf("Nombre de threads:%d\n",omp_get_num_threads());
                  ...
                  uint64_t collapsed, position_list* collapsed_stack)
{
    ...
    #pragma omp parallel for schedule(static,blocks->grid_side)
    for (uint32_t gx=0; gx < blocks->grid_side;gx++){
        row = blk_at(blocks,gx,gy,0,y);
        #pragma omp parallel for shared(collapsed_stack) schedule(static,blocks->block_side)
        for (uint32_t x=0; x < blocks->block_side;x++){
