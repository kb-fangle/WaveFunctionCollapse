...
extern "C" void
cuda_main_loop(wfc_args args, wfc_blocks_ptr init)
{
    bool quit = false;
    ...
    cudaMemcpy(cu_blocks.d_states_init, init->states,
               cu_blocks.sudoku_size * sizeof(uint64_t), cudaMemcpyHostToDevice);
    ...
    while (!quit) {
        ...
        if (solved && args.output_folder != NULL) {
            ...
            cudaMemcpy(blocks->states, cu_blocks.d_states,
                       cu_blocks.sudoku_size * sizeof(uint64_t),
