...
// local functions
// clang-format off
static __device__ void md5_update (struct md5_ctx *);
static __device__ void md5_final  (uint8_t *, struct md5_ctx *);
static __device__ void md5_encode (uint8_t *, struct md5_ctx *);
static __device__ void md5_addsize(uint8_t *, uint32_t, uint32_t);
// clang-format on

// clang-format off
__constant__ unsigned char MD5_PADDING[64] = { /* 512 Bits */
...
__device__ void md5(uint8_t *const M, uint32_t len, uint8_t *digest)
{
    ...
    context.size = 0;
    context.bits = 0;
    context.regs = { 0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476 };
