...
#define bitfield_count __builtin_popcountll
#endif
#endif
...
#define bitfield_count bitfield_count_
#endif
...
static inline
WFC_DEVICE WFC_HOST uint32_t bitfield_get_nth_set_bit(uint64_t v, uint32_t r) {
  uint32_t i = 0;
  while (r > 0) {
    i++;
    r -= v & 1;
